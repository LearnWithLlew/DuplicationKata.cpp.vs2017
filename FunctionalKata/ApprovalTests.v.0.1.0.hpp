...
public:
    ...
    static void verify(ApprovalNamer n, StringWriter s, const Reporter& r) {
        auto extention_with_dot = s.GetFileExtension();
        std::string approvedPath = n.getApprovedFile(extention_with_dot);
        std::string receivedPath = n.getReceivedFile(extention_with_dot);
